...
void _getExtraSettings(string_multimap &argument, convertcontext &context, extra_settings &ext, template_args tpl_args) 
{
    ...
    context.argAddEmoji = argAddEmoji;
    context.argRemoveEmoji = argRemoveEmoji;

    if(!argEmoji.is_undef())
    {
        context.argAddEmoji.set(argEmoji);
        context.argRemoveEmoji.set(true);
    }
    ext.add_emoji = context.argAddEmoji.get(global.addEmoji);
    ext.remove_emoji = context.argRemoveEmoji.get(global.removeEmoji);
    if(ext.add_emoji && ext.emoji_array.empty())
        ext.emoji_array = safe_get_emojis();
    if(!argRenames.empty())
        ext.rename_array = INIBinding::from<RegexMatchConfig>::from_ini(split(argRenames, "`"), "@");
...
outcome::result<std::vector<Proxy>, std::string> _getNodes(convertcontext &context,  string_array lIncludeRemarks, string_array lExcludeRemarks) 
{
...
//    }
    ...
    parse_set.sub_info = &subInfo;
    parse_set.authorized = authorized;
    parse_set.request_header = &context.request_header;
    // parse_set.js_runtime = ext.js_runtime;
    // parse_set.js_context = ext.js_context;
...
{   
    ...
    {   
        std::string argCustomRulesets = urlSafeBase64Decode(getUrlArg(context.init_argument, "ruleset")), argCustomGroups = urlSafeBase64Decode(getUrlArg(context.init_argument, "groups"));
        if(!lSimpleSubscription)
        {
            /// loading custom groups
            if(!argCustomGroups.empty() && !ext.nodelist)
